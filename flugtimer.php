...
<form name="form1" method="post" action="/flugtimer.php">
<table>
 <tr>
...
{
	$pfuschOutput .= "ERFOLG";
	$delete = sql_query("DELETE FROM flugtimer WHERE user = '$_SESSION[user]' AND town = '$_SESSION[city]' AND art = '1'");
	$doit = 1;
}
else
{
	$select = sql_query("SELECT ID, ankunftszeit, flugzeit, notizen FROM flugtimer WHERE art = '1' AND user = '$_SESSION[user]' AND town = '$_SESSION[city]' ORDER BY `ID` ASC");
	$y=0;
	while($row = sql_fetch_array($select))
	{
		...
		$y++;
		if($row['notizen'] == "" && $row['flugzeit'] == "" && $row['ankunftszeit'] == "") $delete = sql_query("DELETE FROM flugtimer WHERE user = '$_SESSION[user]' AND art = '1' AND town = '$_SESSION[city]' && ID = '$y'");
	}
}
$x = 0;
if ($felder == "") { $felder = "5"; }
...
$pfuschOutput .= "<tr>
  <td><input value='$n[$x]' type='text' name='n[$x]' size='26'></td>
  <td><input value='$ankunft[$x]' type='text' name='ankunft[$x]' size='6' onkeyup='funk(this);'></td>
  <td><input value='$zeit[$x]' type='text' name='zeit[$x]' size='6' onkeyup='funk(this);'></td>";
if(0<$x && $x != $felder - 1){
...
if ( $s < "10" ) { $s = '0'.$s; }
	...
	$y = $x+1;
	### Save new data
	$save = sql_query("INSERT INTO `flugtimer` (`ID`, `user`, `town`, `art`, `ankunftszeit`, `flugzeit`, `rueckzeit`, `notizen`) VALUES ($y, '$_SESSION[user]', '$_SESSION[city]', '1', '" . addslashes( $ankunft ) . "', '" . addslashes( $zeit ) . "', '', '" . addslashes( $n ) . "')");
